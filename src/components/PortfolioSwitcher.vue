  ...
  <div class="relative inline-block text-left" ref="dropdownRef">
    <button @click="isOpen = !isOpen" type="button"
      ...
      <div class="flex items-center">
        <!-- Icon -->
        <div class="mr-2">
          <svg v-if="isCombinedView" class="w-4 h-4 text-purple-600" fill="none" viewBox="0 0 24 24"
            stroke="currentColor">
      ...
      <div v-if="isOpen"
        ...
        <div class="py-1">
          ...
          <div v-for="p in portfolios" :key="p.id" @click="handleSelect(p.id)"
            class="flex items-center justify-between w-full px-4 py-2.5 text-sm transition-colors group cursor-pointer"
            :class="[
            ...
            <div class="flex items-center flex-grow">
              <div class="w-2 h-2 rounded-full mr-3 transition-colors"
                :class="currentPortfolioId === p.id ? 'bg-indigo-600' : 'bg-transparent group-hover:bg-gray-300'"></div>
              {{ p.name }}
              <span v-if="p.is_default"
                class="ml-2 px-1.5 py-0.5 text-[10px] bg-gray-100 text-gray-500 rounded border border-gray-200">
                預設
              </span>
            </div>

            <!-- Delete Button (Only for non-default portfolios) -->
            <button v-if="!p.is_default" @click.stop="handleDelete(p)"
              class="p-1.5 text-gray-400 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all rounded-lg hover:bg-red-50"
              title="移除帳戶">
        ...
        <div class="py-1">
          <button @click="handleSelect('combined')"
            class="flex items-center w-full px-4 py-2.5 text-sm transition-colors group" :class="[
              isCombinedView
    ...
    <Teleport to="body">
      <div v-if="showAddModal"
        ...
        <div class="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl animate-in fade-in zoom-in duration-200">
          ...
          <input v-model="newName" type="text" placeholder="例如: 配偶、小孩 A"
...
const { portfolios, currentPortfolioId, currentPortfolio, isCombinedView, selectPortfolio, addPortfolio, deletePortfolio } = usePortfolio()
