  ...
  <div class="min-h-screen bg-gray-50">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div class="mb-10 text-center">
        <h1
          class="text-4xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-purple-600 mb-4 tracking-tight">
          股東會紀念品目錄
        </h1>
      ...
      <!-- Modern Filter Bar -->
      <div
        class="bg-white rounded-2xl shadow-lg border border-gray-100 p-6 mb-10 sticky top-4 z-20 backdrop-blur-md bg-white/90">
        <!-- View Toggle (All / My Collection) -->
        <div class="flex items-center justify-center mb-6 pb-6 border-b border-gray-100">
          <div class="inline-flex p-1 bg-gradient-to-r from-indigo-50 to-purple-50 rounded-xl">
            <button @click="currentView = 'all'"
              class="relative px-8 py-3 rounded-lg text-sm font-bold transition-all duration-300 ease-out flex items-center gap-2"
              :class="currentView === 'all' ? 'bg-white text-indigo-600 shadow-md' : 'text-gray-600 hover:text-gray-800'">
              <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                  d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" />
              </svg>
              全部紀念品
              <span class="ml-1 px-2 py-0.5 bg-gray-100 text-xs rounded-full">{{ gifts.length }}</span>
            </button>
            <button @click="currentView = 'collection'"
              class="relative px-8 py-3 rounded-lg text-sm font-bold transition-all duration-300 ease-out flex items-center gap-2"
              :class="currentView === 'collection' ? 'bg-white text-purple-600 shadow-md' : 'text-gray-600 hover:text-gray-800'">
              <svg class="w-4 h-4" :class="currentView === 'collection' ? 'fill-purple-600' : 'fill-none'"
                viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                  d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
              </svg>
              我的收藏
              <span class="ml-1 px-2 py-0.5 bg-purple-100 text-xs rounded-full">{{ myCollections.length }}</span>
            </button>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <div class="md:col-span-3 relative group">
            ...
            <div class="relative">
              <svg
                class="w-5 h-5 absolute left-3 top-3 text-gray-400 group-focus-within:text-indigo-500 transition-colors"
                fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                  d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
              </svg>
              <input v-model="filters.search" type="text" placeholder="輸入公司名稱、代號或紀念品關鍵字..."
        ...
        <div class="mt-4 flex justify-between items-center border-t border-gray-100 pt-4">
          <div class="flex items-center space-x-2">
            <span v-if="hasActiveFilters"
              class="text-xs font-medium text-indigo-600 bg-indigo-50 px-2 py-1 rounded-md cursor-pointer hover:bg-indigo-100 transition-colors"
              @click="clearFilters">
              清除所有篩選 ✕
            </span>
          </div>
      ...
      <div v-else-if="gifts.length === 0"
        ...
        <button @click="clearFilters"
          class="mt-6 px-6 py-2 bg-indigo-600 text-white rounded-full hover:bg-indigo-700 transition-colors shadow-lg shadow-indigo-200">
          清除所有篩選
        </button>
      </div>
      ...
      <div v-else>
        <TableView :items="mappedGifts" :isExpired="isExpired" @toggle-collection="handleToggleCollection" />
      </div>
    </div>
  </div>
</template>
...
const filteredGifts = computed(() => {
  if (currentView.value === 'collection') {
    // Filter to only show collected items
    const collectionIds = myCollections.value.map(c => c.souvenir_id)
    return gifts.value.filter(g => collectionIds.includes(g.id))
  }
  return gifts.value
})
