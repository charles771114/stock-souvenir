    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10 animate-fade-in">
      ...
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-12 animate-fade-in-up delay-100">
        <div class="glass-card p-6 group">
          <span
            class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4 block leading-none group-hover:text-brand-primary transition-colors">總紀錄筆數</span>
        ...
        <div class="glass-card p-6 group flex flex-col justify-between">
          <span
            class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4 block leading-none group-hover:text-brand-primary transition-colors">年度篩選</span>
      ...
      <div
        ...
        <div class="absolute inset-y-0 left-8 flex items-center pointer-events-none z-10">
          <svg class="w-4 h-4 text-slate-400 group-focus-within/input:text-brand-primary transition-colors" fill="none"
      ...
      <div v-if="loading" class="py-24 flex flex-col items-center gap-6">
        ...
        <p class="text-[11px] font-black text-slate-300 uppercase tracking-[0.2em] animate-pulse">正在存取主資料庫...</p>
      </div>
      ...
      <div v-else class="animate-fade-in-up delay-200">
        ...
        <div class="glass-card overflow-hidden p-0 border-none">
              ...
              <thead>
                <tr class="bg-amber-50 border-b border-amber-100">
                  ...
                  <th
                    ...
                    圖片</th>
                  <th
                    class="px-8 py-5 text-right text-[10px] font-black text-brand-primary/60 uppercase tracking-widest leading-none pr-10">
                    管理功能</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-slate-50">
                <tr v-for="item in paginatedSouvenirs" :key="item.id"
                  class="group hover:bg-amber-50 transition-all duration-300">
                  <td class="px-8 py-5">
                    <span
                      class="px-3 py-1.5 rounded-xl bg-white border border-slate-100 text-brand-primary font-mono text-xs font-black shadow-sm group-hover:border-amber-200 transition-all">
                      {{ item.code }}
                    </span>
                  </td>
                  <td class="px-6 py-5">
                  ...
                  <td class="px-6 py-5">
                    <div class="flex flex-col gap-1">
                      ...
                      <div class="flex items-center gap-1.5">
                        ...
                        <span class="text-[10px] font-bold text-brand-primary/60 font-mono italic">{{ item.last_buy_date ||
                          '-' }}</span>
                      </div>
                    </div>
                  </td>
          ...
          <div class="lg:hidden p-6 sm:p-8 space-y-4">
            <div v-for="item in paginatedSouvenirs" :key="item.id"
              class="p-6 bg-white border border-slate-100 rounded-[2rem] shadow-sm hover:border-amber-200 transition-all active:scale-[0.98]">
              <div class="flex items-center justify-between mb-4">
                <div class="flex items-center gap-3">
                  <div
                  <div
                    class="px-2 py-1 rounded-lg bg-brand-primary text-white font-mono text-[10px] font-black shadow-lg shadow-2xl">
                    {{ item.code }}</div>
              ...
              <div class="flex items-center justify-between text-[10px] font-black uppercase tracking-widest">
                ...
                <div class="text-amber-400">買進: <span class="text-brand-primary/60 font-mono italic">{{ item.last_buy_date
                  || '-' }}</span></div>
              </div>
            </div>
          </div>
        ...
        <div v-if="totalPages > 1" class="mt-12 flex flex-col sm:flex-row items-center justify-between gap-8">
          <div class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">
            目前顯示第 <span class="text-brand-primary">{{ (currentPage - 1) * pageSize + 1 }}</span> 至 <span
              class="text-brand-primary">{{ Math.min(currentPage * pageSize, filteredSouvenirs.length) }}</span> 筆，共 <span
              class="text-brand-primary">{{ filteredSouvenirs.length }}</span> 筆紀錄
          </div>

          <div class="flex items-center gap-1.5 overflow-x-auto pb-4 sm:pb-0 max-w-full custom-scrollbar">
            <button @click="currentPage--" :disabled="currentPage === 1"
              class="h-10 w-10 flex items-center justify-center rounded-xl bg-white border border-slate-100 text-slate-400 hover:bg-amber-50 hover:text-brand-primary transition-all disabled:opacity-30">
            ...
            <button v-for="p in visiblePages" :key="p" @click="currentPage = p"
              ...
              :class="currentPage === p ? 'bg-brand-primary text-white shadow-xl shadow-2xl' : 'bg-white border border-slate-100 text-slate-400 hover:bg-slate-50'">
              {{ p }}
            </button>
        ...
        <div class="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-xl relative overflow-hidden animate-bounce-in">
          <div class="p-8 sm:p-12">
            <div class="flex items-center justify-between mb-10">
              <div>
                <span class="text-[10px] font-black text-brand-primary uppercase tracking-widest mb-1 block">資料管理</span>
            ...
            <form @submit.prevent="saveItem" class="space-y-6">
              <div class="grid grid-cols-2 gap-5">
                <div class="space-y-2">
                  ...
                  <input v-model="formData.code" type="text" required :disabled="isEditing"
                    class="w-full h-14 px-6 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-black focus:bg-white focus:border-amber-200 transition-all outline-none" />
                </div>
                <div class="space-y-2">
              ...
              <div class="mt-10 flex gap-4">
                ...
                <button type="submit" :disabled="saving"
                  class="flex-[2] h-14 bg-brand-primary text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-2xl hover:bg-amber-600 transition-all flex items-center justify-center gap-2">
                  {{ saving ? '正在儲存...' : '確認儲存資料' }}
                </button>
              </div>
            </form>
