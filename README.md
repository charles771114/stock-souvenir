...
- **通知自動化與管理 (Notifications)**：
    - **智慧排程**：整合 `pg_cron` 於每日 09:00 及 13:00 自動推播最新資訊。
    - **動態校準**：早上發送全量預告，下午 13:00 僅針對「今日截止」項目進行最終催促。
    - **內容去重 (Smart Redundancy)**：自動記錄發送快照，內容未變動時自動跳過，但若有 **「今日截止」** 或 **「超商禮券」** 項目則發送精簡版緊急通知。
    - **更新偵測**：自動偵測「尚未公布」更新為具體項目的紀念品。
    - **配額監控 (Quota Monitor)**：後台即時顯示 LINE Messaging API 的本月賸餘發送額度。
    - **管理後台**：提供即時訊息預覽、手動發送工具及 LINE Token 有效性驗證。

### ⚡ 零額度方案 (Zero-Quota Alternative)
- **今日重點頁面 (/today)**：公開、免登入、行動版優化的重點快訊頁面。
    - **截止時間軸**：精準收錄未來 7 天內（含今天）即將截止的標的。
    - **情報更新**：同步顯示最近 48 小時內有異動或新增的紀念品資訊。
- **LINE 關鍵字查詢**：機器人自動偵測「最新」、「紀念品」等關鍵字，秒回 `/today` 網頁連結，完全不消耗 LINE Push API 額度。
...
### 🛡️ 管理員操作指南
- **進入後台**：登入具管理員權限的帳號後，點擊導航列的「管理員」選單。
- **通知管理**：
    - 前往「通知管理中心」。
    - 點擊「生成預覽」查看 LINE 訊息草稿。
    - 點擊「測試 LINE Token 有效性」確保 API 連線正常。
    - 若有急需，可點擊「立即執行」發送即時廣播。
2. **批量匯入資料**：
    - 前往「庫存歸戶管理」。
